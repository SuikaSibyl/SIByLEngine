#ifndef _WOS_BUNNY_EXAMPLE_2D_HLSLI
#define _WOS_BUNNY_EXAMPLE_2D_HLSLI

#include "../core/pde.hlsli"
#include "../core/source.hlsli"
#include "common/math.hlsli"

struct Bunny2D_PDEImpl : IPDEImpl<2, 3> {
    ...
    static const SphereSource2DList<3> sphere_sources = {
        {
            { float2(0.0, 0.0), 0.05, float3(400, 0.2, 0.2) },
            { float2(0.2, 0.1), 0.05, float3(0.2, 400, 0.2) },
            { float2(0.2, -0.1), 0.05, float3(0.2, 0.2, 400) },
        }
    };
    ...
    static float3 dirichlet_impl(input_t x) {
        return 0.8;
    }

    // returns the source value at the input point
    static float3 source_impl(input_t x) {
        return sphere_sources.source(x);
    }
    ...
    static source_sample<2, 3> sample_source_impl(uint seed) {
        ...
        source_sample<2, 3> test;
        return sphere_sources.sample_source(GetNextRandomFloat3(rng));
    }
    ...
    static float pdf_source_impl(float2 p) {
        return sphere_sources.pdf_source(p);
    }

    // returns the absorption coefficient value at the input point
    static float3 absorption_impl(input_t x) {
        return 0.8f;
    }
...
struct BunnyBoundary2D : IBoundary<2, 3> {
    static float distance(float2 x) {
        return domain_bunny_2d(x);
    }

    // return the closest point on the boundary to the point x
    static float2 closest_point(float2 x) { return float2(0, 0); }
...
float domain_bunny_2d(float2 p_2d) {
    float3 p = float3(p_2d, 0);
    float3 op = p;
    p = op.zxy * 0.75;
...
#endif // _WOS_BUNNY_EXAMPLE_2D_HLSLI